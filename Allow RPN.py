...
def Main():
    ...
    PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber, useRPN)
    input()
    
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber, useRPN):
    Score = 0
    GameOver = False
    while not GameOver:
        ...
        print()
        if CheckIfUserInputValid(UserInput) or useRPN:
            if useRPN:
                UserInputInRPN = list(UserInput)
            else:
